...
func (_q *CommentQuery) QueryCase() *CaseRecordQuery {
...
func (_q *CommentQuery) QueryEvidence() *EvidenceQuery {
...
func (_q *CommentQuery) WithCase(opts ...func(*CaseRecordQuery)) *CommentQuery {
	query := (&CaseRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) WithEvidence(opts ...func(*EvidenceQuery)) *CommentQuery {
	query := (&EvidenceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if _q.withAuthor != nil || _q.withCase != nil || _q.withEvidence != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withCase; query != nil {
		if err := _q.loadCase(ctx, query, nodes, nil,
			func(n *Comment, e *CaseRecord) { n.Edges.Case = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEvidence; query != nil {
		if err := _q.loadEvidence(ctx, query, nodes, nil,
			func(n *Comment, e *Evidence) { n.Edges.Evidence = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CommentQuery) loadCase(ctx context.Context, query *CaseRecordQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *CaseRecord)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].case_record_comments == nil {
			continue
		}
		fk := *nodes[i].case_record_comments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "case_record_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *CommentQuery) loadEvidence(ctx context.Context, query *EvidenceQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Evidence)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
