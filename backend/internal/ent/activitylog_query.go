...
func (_q *ActivityLogQuery) QueryUser() *UserQuery {
...
func (_q *ActivityLogQuery) QueryCase() *CaseRecordQuery {
...
func (_q *ActivityLogQuery) WithUser(opts ...func(*UserQuery)) *ActivityLogQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *ActivityLogQuery) WithCase(opts ...func(*CaseRecordQuery)) *ActivityLogQuery {
	query := (&CaseRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ActivityLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ActivityLog, error) {
	...
	if _q.withUser != nil || _q.withCase != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withCase; query != nil {
		if err := _q.loadCase(ctx, query, nodes, nil,
			func(n *ActivityLog, e *CaseRecord) { n.Edges.Case = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ActivityLogQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*ActivityLog, init func(*ActivityLog), assign func(*ActivityLog, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*ActivityLog)
	for i := range nodes {
		if nodes[i].user_activity == nil {
			continue
		}
		fk := *nodes[i].user_activity
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_activity" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ActivityLogQuery) loadCase(ctx context.Context, query *CaseRecordQuery, nodes []*ActivityLog, init func(*ActivityLog), assign func(*ActivityLog, *CaseRecord)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*ActivityLog)
	for i := range nodes {
		if nodes[i].case_record_activity == nil {
			continue
		}
		fk := *nodes[i].case_record_activity
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "case_record_activity" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
