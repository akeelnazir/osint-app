...
func (_q *EntityQuery) QueryEvidence() *EvidenceQuery {
...
func (_q *EntityQuery) WithEvidence(opts ...func(*EvidenceQuery)) *EntityQuery {
	query := (&EvidenceClient{config: _q.config}).Query()
...
func (_q *EntityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Entity, error) {
	...
	if query := _q.withEvidence; query != nil {
		if err := _q.loadEvidence(ctx, query, nodes, nil,
			func(n *Entity, e *Evidence) { n.Edges.Evidence = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EntityQuery) loadEvidence(ctx context.Context, query *EvidenceQuery, nodes []*Entity, init func(*Entity), assign func(*Entity, *Evidence)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Entity)
	for i := range nodes {
		if nodes[i].evidence_entities == nil {
			continue
		}
		fk := *nodes[i].evidence_entities
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "evidence_entities" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
