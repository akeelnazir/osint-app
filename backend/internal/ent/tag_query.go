...
func (_q *TagQuery) QueryCases() *CaseRecordQuery {
...
func (_q *TagQuery) QueryEvidence() *EvidenceQuery {
...
func (_q *TagQuery) WithCases(opts ...func(*CaseRecordQuery)) *TagQuery {
	query := (&CaseRecordClient{config: _q.config}).Query()
...
func (_q *TagQuery) WithEvidence(opts ...func(*EvidenceQuery)) *TagQuery {
	query := (&EvidenceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withCases; query != nil {
		if err := _q.loadCases(ctx, query, nodes,
			func(n *Tag) { n.Edges.Cases = []*CaseRecord{} },
			func(n *Tag, e *CaseRecord) { n.Edges.Cases = append(n.Edges.Cases, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEvidence; query != nil {
		if err := _q.loadEvidence(ctx, query, nodes,
			func(n *Tag) { n.Edges.Evidence = []*Evidence{} },
			func(n *Tag, e *Evidence) { n.Edges.Evidence = append(n.Edges.Evidence, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadCases(ctx context.Context, query *CaseRecordQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *CaseRecord)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.CasesTable)
		s.Join(joinT).On(s.C(caserecord.FieldID), joinT.C(tag.CasesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.CasesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.CasesPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*CaseRecord](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "cases" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *TagQuery) loadEvidence(ctx context.Context, query *EvidenceQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Evidence)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Tag)
	nids := make(map[int]map[*Tag]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.EvidenceTable)
		s.Join(joinT).On(s.C(evidence.FieldID), joinT.C(tag.EvidencePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.EvidencePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.EvidencePrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
