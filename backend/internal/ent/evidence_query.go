...
func (_q *EvidenceQuery) QueryCase() *CaseRecordQuery {
...
func (_q *EvidenceQuery) QueryCreator() *UserQuery {
...
func (_q *EvidenceQuery) QueryTags() *TagQuery {
...
func (_q *EvidenceQuery) QueryEntities() *EntityQuery {
...
func (_q *EvidenceQuery) QueryComments() *CommentQuery {
...
func (_q *EvidenceQuery) WithCase(opts ...func(*CaseRecordQuery)) *EvidenceQuery {
	query := (&CaseRecordClient{config: _q.config}).Query()
...
func (_q *EvidenceQuery) WithCreator(opts ...func(*UserQuery)) *EvidenceQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EvidenceQuery) WithTags(opts ...func(*TagQuery)) *EvidenceQuery {
	query := (&TagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EvidenceQuery) WithEntities(opts ...func(*EntityQuery)) *EvidenceQuery {
	query := (&EntityClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EvidenceQuery) WithComments(opts ...func(*CommentQuery)) *EvidenceQuery {
	query := (&CommentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EvidenceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Evidence, error) {
	...
	if _q.withCase != nil || _q.withCreator != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withCase; query != nil {
		if err := _q.loadCase(ctx, query, nodes, nil,
			func(n *Evidence, e *CaseRecord) { n.Edges.Case = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCreator; query != nil {
		if err := _q.loadCreator(ctx, query, nodes, nil,
			func(n *Evidence, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTags; query != nil {
		if err := _q.loadTags(ctx, query, nodes,
			func(n *Evidence) { n.Edges.Tags = []*Tag{} },
			func(n *Evidence, e *Tag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEntities; query != nil {
		if err := _q.loadEntities(ctx, query, nodes,
			func(n *Evidence) { n.Edges.Entities = []*Entity{} },
			func(n *Evidence, e *Entity) { n.Edges.Entities = append(n.Edges.Entities, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withComments; query != nil {
		if err := _q.loadComments(ctx, query, nodes,
			func(n *Evidence) { n.Edges.Comments = []*Comment{} },
			func(n *Evidence, e *Comment) { n.Edges.Comments = append(n.Edges.Comments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EvidenceQuery) loadCase(ctx context.Context, query *CaseRecordQuery, nodes []*Evidence, init func(*Evidence), assign func(*Evidence, *CaseRecord)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Evidence)
	for i := range nodes {
		if nodes[i].case_record_evidence == nil {
			continue
		}
		fk := *nodes[i].case_record_evidence
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "case_record_evidence" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EvidenceQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Evidence, init func(*Evidence), assign func(*Evidence, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Evidence)
	for i := range nodes {
		if nodes[i].user_created_evidence == nil {
			continue
		}
		fk := *nodes[i].user_created_evidence
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_created_evidence" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EvidenceQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Evidence, init func(*Evidence), assign func(*Evidence, *Tag)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Evidence)
	nids := make(map[int]map[*Evidence]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(evidence.TagsTable)
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(evidence.TagsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(evidence.TagsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(evidence.TagsPrimaryKey[0]))
...
func (_q *EvidenceQuery) loadEntities(ctx context.Context, query *EntityQuery, nodes []*Evidence, init func(*Evidence), assign func(*Evidence, *Entity)) error {
	...
	query.Where(predicate.Entity(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(evidence.EntitiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.evidence_entities
		if fk == nil {
			return fmt.Errorf(`foreign-key "evidence_entities" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "evidence_entities" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *EvidenceQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Evidence, init func(*Evidence), assign func(*Evidence, *Comment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Evidence)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
